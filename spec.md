...
- 設定可能なグリッドシステムによる直感的な位置指定（デフォルト12×9）
- 標準Markdown記法との完全互換
- スタイルクラスによる柔軟なスタイリング
- 位置指定省略時の自動レイアウト
- 監視モードによる自動再生成
...
| `theme` | string \| object | テーマ名またはテーマ設定オブジェクト | `default` |
...
- **行**: 1〜グリッド行数（上から下）
- 座標は1始まり（1-indexed）
- グリッドサイズは`grid`プロパティで設定可能（デフォルト12×9）
...
アニメーション効果を指定できます。PptxGenJSのアニメーションタイプを使用します。

```markdown
# タイトル {animation=fadein}
テキスト {animation=flyin duration=1 delay=0.5}
```

| プロパティ | 説明 | 単位 | デフォルト |
|-----------|------|------|-----------|
| `animation` | アニメーションタイプ | - | なし |
| `duration` | 再生時間 | 秒 | 1 |
| `delay` | 開始遅延 | 秒 | 0 |
| `direction` | 方向 | - | タイプによる |
| `trigger` | トリガー | `onClick`, `afterPrevious`, `withPrevious` | `onClick` |
| `repeat` | 繰り返し回数 | 回数 | 1 |
| `speed` | 速度 | `slow`, `medium`, `fast` | `medium` |
...
interface GridPosition {
  colStart: number;  // 列開始（1〜グリッド列数）
  colEnd: number;    // 列終了（1〜グリッド列数）
  rowStart: number;  // 行開始（1〜グリッド行数）
  rowEnd: number;    // 行終了（1〜グリッド行数）
}
```
