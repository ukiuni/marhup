...
describe('MCP Tools', () => {
  describe('getToolDefinitions', () => {
    ...
    it('get_marhup_guideツールが定義されている', () => {
      const tools = getToolDefinitions();
      const tool = tools.find((t) => t.name === 'get_marhup_guide');
  ...
  describe('getMarhupGuide', () => {
    it('ガイド文字列を返す', () => {
      const guide = getMarhupGuide();

      expect(guide).toContain('marhup Markdown記法ガイド');
  ...
  describe('handleToolCall', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'marhup-test-'));
    });
    ...
    describe('get_marhup_guide', () => {
      it('ガイドを返す', async () => {
        const result = await handleToolCall('get_marhup_guide', {});

        expect(result.isError).toBeUndefined();
        expect(result.content).toHaveLength(1);
        expect(result.content[0].text).toContain('marhup Markdown記法ガイド');
      });
    });
